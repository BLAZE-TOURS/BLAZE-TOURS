...
function addMeditationType() {
    var form = document.getElementById('addMeditationTypeForm');
    var formData = new FormData(form);

    var validationErrors = document.getElementById('validation-errors-meditation');
    var successMessage = document.getElementById('success-message-meditation');
    var loadingSpinner = document.getElementById('loading-spinner-meditation');
    ...
    r.onreadystatechange = function () {
        if (r.readyState == 4) {
            ...
            if (r.status == 200) {
                if (t.includes("Meditation type added successfully")) {
                    successMessage.innerHTML = t;
                    successMessage.classList.remove('d-none');
                    validationErrors.classList.add('d-none');
                    form.reset(); // Clear the form fields
                    document.getElementById('meditation_image_preview').style.display = 'none';
                    var modal = bootstrap.Modal.getInstance(document.getElementById('addMeditationTypeModal'));
                    modal.hide(); // Hide the modal
                    Swal.fire(
                        'Success!',
                        'Meditation type added successfully.',
                        'success'
                    ).then(() => {
    ...
    r.open("POST", "../process/addMediTypeProcess.php", true);
    r.send(formData);
}
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            r.onreadystatechange = function () {
                if (r.readyState == 4 && r.status == 200) {
                    var data = JSON.parse(r.responseText);
                    document.getElementById('update_news_id').value = data.id;
                    document.getElementById('update_news_name').value = data.title;
                    document.getElementById('update_news_start_date').value = data.date_time_start;
                    document.getElementById('update_news_end_date').value = data.date_time_end;
                    document.getElementById('update_news_description').value = data.description;
                    // Show current image
                    var imgContainer = document.getElementById('current-image-container');
                    var previewImg = document.getElementById('update_news_image_preview');
                    if (data.img_url) {
                        imgContainer.innerHTML = '<span>Current Image:</span><br><img src="images/news/' + data.img_url + '" style="max-width:100px;">';
                        previewImg.src = 'images/news/' + data.img_url;
                        previewImg.style.display = 'block';
                    } else {
                        imgContainer.innerHTML = '';
                        previewImg.style.display = 'none';
                    }
                    // Add onchange handler to preview new image
                    var fileInput = document.getElementById('update_news_image');
                    if (fileInput) {
                        fileInput.onchange = function(event) {
                            previewImage(event, 'update_news_image_preview');
                        };
                    }
                    // Show modal
            ...
            r.open("GET", "../process/getNewsById.php?id=" + id, true);
            r.send();
        }
    });
}
...
function submitUpdateNews() {
    var form = document.getElementById('updateNewsTypeForm');
    var formData = new FormData(form);

    var validationErrors = document.getElementById('validation-errors-news-update');
    var successMessage = document.getElementById('success-message-news-update');
    var loadingSpinner = document.getElementById('loading-spinner-news-update');
    ...
    r.onreadystatechange = function () {
        if (r.readyState == 4) {
            ...
            var t = r.responseText;
            if (r.status == 200) {
                if (t.includes("News updated successfully")) {
                    successMessage.innerHTML = t;
                    successMessage.classList.remove('d-none');
                    validationErrors.classList.add('d-none');
    ...
    loadingSpinner.classList.remove('d-none');
    r.open("POST", "../process/updateNewsProcess.php", true);
    r.send(formData);
}
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            r.onreadystatechange = function () {
                if (r.readyState == 4) {
                    var t = r.responseText;
                    if (r.status == 200 && t.includes("News deleted successfully")) {
                        Swal.fire(
                            'Deleted!',
            ...
            r.open("POST", "../process/deleteNewsProcess.php", true);
            r.send(formData);
        }
    });
}
        ...
        if (r.readyState == 4) {
            ...
            if (r.status == 200) {
                if (t.includes("News added successfully")) {
                    ...
                    var modal = bootstrap.Modal.getInstance(document.getElementById('addNewsModal'));
                    modal.hide();
                    Swal.fire(
                        'Success!',
