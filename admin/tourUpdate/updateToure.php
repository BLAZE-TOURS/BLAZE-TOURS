    ...
    <body data-menu-color="light" data-sidebar="default">
        ...
        <div class="mt-3">
            ...
            `;
                ...
                // `// Fetch tour data by ID from URL and show in alert`
                // This script will run when the page loads
                $(document).ready(function() {
                    const urlParams = new URLSearchParams(window.location.search);
                    const tourId = urlParams.get('id');
                    if (tourId) {
                        fetch('../tourUpdate/fetchTourById.php?id=' + encodeURIComponent(tourId))
                            .then(response => response.json())
                            .then(data => {
                                if (data && !data.error) {
                                    alert("Tour data loaded:\n" + JSON.stringify(data, null, 2));
                                } else {
                                    alert("No data found for this tour id.\n" + (data.error || 'Unknown error'));
                                }
                            })
                            .catch((e) => {
                                alert('Error fetching tour data: ' + e);
                            });
                    } else {
                        alert('No tourId in URL');
                    }
                });
            </script>
        ...
        <script src="../tourUpdate/assets/js/update.js"></script>
